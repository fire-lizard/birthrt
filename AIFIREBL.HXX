...
inline void FIREBALL_DATA::mfInitVals()
{
	Movement.dx = 0;
	Movement.dy = 0;
	Movement.dz = 0;
...
inline void FIREBALL_DATA::mfWriteData(FILE *fp) const
{
	fprintf(fp, "%ld %ld\n", Movement.dx, Movement.dy);
}

// read member data from scene file
inline const LONG FIREBALL_DATA::mfReadData(FILE *fp)
{
	char cpBuffer[80];
