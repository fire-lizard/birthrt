...
#if 0 // GWP this version isn't used.
void xscale_col_ttop(long sx,long dx,long dy,long dye,
				long clipped,long src_inc)
{
	LONG			tsy = (tyoff << 16) + (clipped * src_inc);
	LONG			light;
	LONG			wrapMask;
	LONG			sptr_inc;
	PTR			sptr;
	...
	if (light > DARKEST)
		light = DARKEST;
   RenderChangeLight (light);  // apply global lighting effects
	// if (gfInfraVision)
	// 	light += INFRAVISION_OFFSET;
	// SetLight((light>DARKEST) ? DARKEST: ((light<0)?0:light));
	SetLight (light);
...
|	{
|		tsy += ((wTexture - (floor_z % wTexture)) << 16);
|	}
|	else if (gPeggedValue & 16)         // "peg low" = peg to floor
|	{
|		tsy += ((wTexture - (col_hei % wTexture)) << 16);
|	}
...
|	}
	...
	// JPC note: from the following business with "wrapMask" we can extract a
	//    RULE: texture width MUST be a power of 2.
	// (Otherwise we would need to use % instead of & in the "do" loop,
	// and % is slower.)
	wrapMask = (wTexture << 16) - 1;		/* make texture wrap mask */
	tsy &= wrapMask;							/* texture wraps at wrapMask */
