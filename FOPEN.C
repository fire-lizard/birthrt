...
int __far crit_error_handler(unsigned deverr, unsigned errcode, unsigned far *devhdr)
{
	...
	// if we haven't initialized the video, then just leave
	// this will be the call to CheckCD before anything gets initialized
	// the message will be printed in CheckCD below
	if(fGraphInitialized == FALSE)
		return(_HARDERR_IGNORE);

	// save the state of the fade
	fFadeOut = fIsFadedOut;

	// if we are faded out, fade in first
	if(fIsFadedOut = TRUE)
	{
		FadeIn(0);
	}

	// put up a box telling the user to put in the birthright CD
//	sprintf(szText, "^F02%s", STRMGR_GetStr(STR_REPLACE_CD));
	sprintf(szText, "^F02%s", "Please insert your Birthright CD, and press a key.");
	w = gtext_width(szText) + 8;
	h = 20;
	x = 320 - (w/2);
	y = 240 - (h/2);
	color_rect(x, y, w, h, 223);            // lt tan
	color_box(x, y, w, h, 223-8);           // md tan
	print_textf(x + 4, y + 4, 128, szText); // dk brown
...
void CheckCD(void)
{
	...
	if( '\0' != CDDrive[0] )
			...
			sprintf(chFileName, "%sBIRTHRT\\%s", CDDrive, "birth.url");
		#else
			sprintf(chFileName, "%s%s", CDDrive, "birth.url");
		#endif
...
checkit:
	if( ( access( chFileName, ( F_OK ) ) ) != 0 )
	{
		// ask for CD
#ifdef _WINDOWS
		if(MessageBox (NULL, "Please insert your Birthright CD.  Click Cancel to exit.", "Birthright", MB_OKCANCEL | MB_ICONINFORMATION | MB_APPLMODAL) == IDCANCEL)
		{
			quit_sys(0L);
			return;
		}
#else
		// if we haven't initialized the video, then just print to the screen
		if(fGraphInitialized == FALSE)
		{
			cprintf("Please insert your Birthright CD, and press a key.  Press Q to quit.\r\n");
			while(TRUE)
			{
				if(kbhit())
				{
